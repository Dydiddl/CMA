...
class TestUserService:
    ...
    def test_get_user_by_id_success(self, user_service, sample_user_model):
        """ID로 사용자 조회 성공 테스트"""
        # Given
        user_id = "user-123"
        ...
        assert result.full_name == sample_user_model.full_name
    ...
    def test_get_user_by_email_success(self, user_service, sample_user_model):
        """이메일로 사용자 조회 성공 테스트"""
        # Given
        email = "test@example.com"
    ...
    def test_create_user_success(self, user_service, sample_user_data):
        ...
        with patch.object(user_service, 'get_user_by_email', return_value=None):
            ...
            user_service.db.refresh.side_effect = lambda user: setattr(user, 'id', 'user-123')
    ...
    def test_get_user_stats_success(self, user_service):
        ...
        user_service.db.query.return_value = mock_query
        mock_query.filter.return_value = mock_query
        mock_query.count.side_effect = [total_users, active_users, 5, 3]  # total, active, admin_count, user_count
        mock_query.distinct.return_value = mock_query
        mock_query.all.return_value = roles_data
        ...
        assert result["active_users"] == active_users
        assert result["inactive_users"] == inactive_users
