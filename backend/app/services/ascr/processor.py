...
print(
    f"[INFO] 실행 플랫폼: {platform_info['os']} / Python {platform_info['python_version']}"
)
...
class ASCRProcessor:
        ...
        try:
            ...
            try:
                ...
                pdf_info = await loop.run_in_executor(
                    self.thread_pool, self._get_pdf_info, str(temp_path)
                )
                    ...
                    if "제" in text and "장" in text:
                        ...
                        for line in lines:
                            if "제" in line and "장" in line:
                                chapters.append(
                                    {"title": line.strip(), "page": page_num}
                                )
                                break
    ...
    async def extract_text(
        self, pdf_file, pages: Optional[List[int]] = None
    ) -> Dict[str, Any]:
        """
        PDF 텍스트 추출
    ...
    async def split_pdf(self, pdf_file, toc_structure: dict) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                if not chapters:
                    self.logger.warning(
                        "목차 정보가 없어 전체 PDF를 하나의 파일로 저장합니다."
                    )
                    # 전체 PDF를 하나의 파일로 저장
                    output_file = self.output_dir / f"complete_document.pdf"
                    with open(
                        output_file, "wb", encoding="utf-8", newline=""
                    ) as output:
                        writer = PdfWriter()
                    ...
                    for i, chapter in enumerate(chapters):
                        ...
                        output_file = (
                            self.output_dir
                            / f"{safe_title}_{chapter_page}-{end_page+1}.pdf"
                        )
    ...
    async def analyze_ground_truth(
        self, data_file, analysis_type: str = "comprehensive"
    ) -> Dict[str, Any]:
        """
        지반 진실 데이터 분석
    ...
    async def fix_hierarchy(
        self, structure_file, fix_type: str = "dots_to_commas"
    ) -> Dict[str, Any]:
        """
        계층 구조 수정
