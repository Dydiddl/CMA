...
class FinalHierarchyStructureFixer:
    """최종 개선된 계층 구조 수정 클래스"""
        ...
        try:
            ...
            result_data = {
                ...
                "metadata": {
                    ...
                    "fix_method": "final_hierarchy_structure_fixer"
                }
            }
    ...
    def _print_statistics(self, entries: List[Dict]) -> None:
        ...
        print("\n=== 샘플 구조 ===")
        sample_count = 0
        for entry in entries[:20]:  # 처음 20개만 출력
            level = entry.get('level', 0)
            indent = "  " * level
            number = entry.get('number', '')
            title = entry.get('title', '')
            section = entry.get('section', '')
            print(f"{indent}{number} {title} ({section})")
            sample_count += 1
            if sample_count >= 20:
                break
...
def main():
    ...
    output_file = Path("output/toc_structure_fixed_final.json")
    ...
    # 계층 구조 수정 실행
    fixer = FinalHierarchyStructureFixer()
